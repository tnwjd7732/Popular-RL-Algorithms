        ...
        if(len(self.cluster)) < action2 and (action1 != 1):
            ...
            if tasktime < Ttotal: #실패
                ...
                if math.isinf(Ttotal):
                    if math.isinf(Tloc):
                        r1 = -3  # Penalty for incorrect local fraction decision
                    if math.isinf(Toff):
                ...
                reward = max(1, reward /5)
               
                #print(abs(Tloc-Toff), Tloc, Toff)
                r1 = min(max(1, reward),2)
                r2 = min(max(1, reward),2)
            ...
            if Tloc != 0:
                ...
                self.alloRes_loc[stepnum] = 0
                param.remains[self.nearest] -= 0
        else:
            if(len(self.cluster)) < action2: #invalid action
                action_globid_ver=0
