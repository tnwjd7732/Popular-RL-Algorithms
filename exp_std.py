...
def plot(results, std_range):
    ...
    # Proposed와 관련된 스킴들: 같은 색 (보라색) + 서로 다른 선 스타일 및 심볼
    ax1.plot(std_range, results['our_succ'], label="Proposed", color='purple', linestyle='-', marker='o', linewidth=2)
    ax1.plot(std_range, results['woclst_succ'], label="Without Clustering", color='purple', linestyle='--', marker='s', linewidth=2)
    ax1.plot(std_range, results['staticclst_succ'], label="Static Clustering", color='purple', linestyle=':', marker='D', linewidth=2)

    ax1.set_xlabel('Resource Standard Deviation', fontsize=font_size + 10)
    ax1.set_ylabel('Success Rate', fontsize=font_size + 10)
    ax1.legend(loc='best', fontsize=font_size - 3, frameon=False)
    ax1.grid(True, linestyle='--', linewidth=0.5)
    ax1.tick_params(axis='both', which='major', labelsize=font_size)

    # Reward Plot
    ax2.plot(std_range, results['our_reward'], label="Proposed", color='purple', linestyle='-', marker='o', linewidth=2)
    ax2.plot(std_range, results['woclst_reward'], label="Without Clustering", color='purple', linestyle='--', marker='s', linewidth=2)
    ax2.plot(std_range, results['staticclst_reward'], label="Static Clustering", color='purple', linestyle=':', marker='D', linewidth=2)

    ax2.set_xlabel('Resource Standard Deviation', fontsize=font_size + 10)
    ax2.set_ylabel('Average Reward', fontsize=font_size + 10)
    ax2.legend(loc='best', fontsize=font_size - 3, frameon=False)
...
if __name__ == '__main__':
    std_range = np.arange(0.1, 10.2, 2.5)
